...
function generate_board() {
  ...
  for (i = 0; i < num_rows; i++) {
    ...
    for (j = 0; j < num_cols; j++) {
      ...
      col.push(image);
      result_str += `<img class='unsolved' onclick="flip(this, ${i},${j})" src='./cards/hidden.png'/>`;
    }
    board.push(col);
    result_str += "</div>";
  }
...
function flip(elem, row, col) {
  if (flipped.length == 2 || elem.id == "flipped") {
    return;
  }
  var image = board[row][col];
  elem.id = "flipped";
  elem.src = `./cards/${image}`;
  flipped.push(elem);
  if (flipped.length == 2) {
...
function check() {
  if (flipped[0].src == flipped[1].src) {
    ...
    flipped[0].className = "solved";
    flipped[1].className = "solved";
    flipped[0].style.opacity = "0.5";
    flipped[1].style.opacity = "0.5";
  }
  // Reset cards
  var allCards = document.getElementsByClassName("unsolved");
